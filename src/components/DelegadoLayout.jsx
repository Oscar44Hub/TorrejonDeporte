...
const navItems = [
  ...
  { path: '/mi-equipo', label: 'Mi equipo', icon: Shield },
  { path: '/mis-jugadores', label: 'Mis jugadores', icon: Users },
  { path: '/mis-partidos', label: 'Mis partidos', icon: Calendar },
...
export default function DelegadoLayout() {
  ...
  if (!user) {
    base44.auth.redirectToLogin('/mi-panel');
    return null;
  }
    ...
    <div className="min-h-screen bg-background flex">
          ...
          {user && (
            <div className="flex items-center gap-2.5 px-2 py-2 bg-sidebar-accent rounded-lg">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sidebar-foreground/50 text-xs">Delegado</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-card border-b border-border sticky top-0 z-30">
          ...
          <span className="font-oswald font-bold text-primary text-base">Panel Delegado</span>
          <div className="w-9" />
        </header>
