...
export default function Partidos() {
  ...
  useEffect(() => {
    base44.entities.Match.list('-match_date', 200).then(data => {
      setMatches(data);
      setLoading(false);
    });
  }, []);
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center bg-muted rounded-lg p-1 gap-1">
          <button onClick={() => setVista('lista')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${vista === 'lista' ? 'bg-white shadow-sm text-foreground' : 'text-muted-foreground'}`}>
            <List className="w-4 h-4" /> Lista
          </button>
          <button onClick={() => setVista('calendario')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${vista === 'calendario' ? 'bg-white shadow-sm text-foreground' : 'text-muted-foreground'}`}>
                          ...
                          <div className="flex items-center gap-2 text-center flex-shrink-0">
                            {m.status === 'finalizado' || m.status === 'en_juego' ? (
                              ...
                              <div className="px-3 py-1 bg-muted rounded-lg">
                                <p className="text-xs font-bold text-muted-foreground">{m.match_date ? format(new Date(m.match_date), 'HH:mm') : 'TBD'}</p>
                              </div>
                            )}
                          </div>
      ...
      {vista === 'calendario' && (
        <div className="bg-card border border-border rounded-xl p-5">
          {loading ? (
            ...
            <CalendarioVista matches={matches} />
          )}
        </div>
      )}
    </div>
